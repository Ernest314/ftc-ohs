...
void Move_Forward(int angle, int power=g_FullDrivePower)
{
	int currentAngleL = Motor_GetEncoder(motor_L);
	int currentAngleR = Motor_GetEncoder(motor_R);
	int targetAngleL = currentAngleL+angle;
	int targetAngleR = currentAngleR+angle;

	Motor_Target(motor_L, targetAngleL);
	Motor_Target(motor_R, targetAngleR);
	Motor_Forward(motor_L, power);
	Motor_Forward(motor_R, power);

	while ( (abs(Motor_GetEncoder(motor_L)-targetAngleL)>g_MoveTolerance) ||
			(abs(Motor_GetEncoder(motor_R)-targetAngleR)>g_MoveTolerance) )
				{;}
	Motor_Stop(motor_L);
	Motor_Stop(motor_R);
}
...
void Move_Backward(int angle, int power=g_FullDrivePower)
{
	int currentAngleL = Motor_GetEncoder(motor_L);
	int currentAngleR = Motor_GetEncoder(motor_R);
	int targetAngleL = currentAngleL-angle;
...
void Turn_Right(int angle, int steerL, int steerR)
{
	int currentAngleL = Motor_GetEncoder(motor_L);
	int currentAngleR = Motor_GetEncoder(motor_R);
	int targetAngleL = currentAngleL+angle;
...
void Turn_Left(int angle, int steerL, int steerR)
{
	int currentAngleL = Motor_GetEncoder(motor_L);
	int currentAngleR = Motor_GetEncoder(motor_R);
	int targetAngleL = currentAngleL-angle;
...
void Lift_Up(int height, int power)
{
...
void Lift_Down(int height, int power)
{
	Motor_Reverse(motor_lift, power);
	Time_Wait(height);
	Motor_Stop(motor_lift);
}
